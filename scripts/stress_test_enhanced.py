...
class EnhancedStressTester:
    ...
    def __init__(self, base_url: str, github_secret: str = "test-secret", 
                 gitee_secret: str = "test-secret", notion_secret: str = "test-secret"):
    ...
    def generate_notion_payload(self, page_number: int) -> Tuple[Dict[str, Any], str]:
        ...
        payload = {
            ...
            "properties": {
                "Title": {
                    "title": [{"text": {"content": f"Notion Stress Test {page_number}"}}]
                },
                "Status": {
    ...
    async def send_webhook_request(self, session: aiohttp.ClientSession, 
                                 provider: str, payload: Dict[str, Any], 
                                 headers: Dict[str, str], delivery_id: str) -> Dict[str, Any]:
        """发送单个webhook请求"""
    ...
    async def run_concurrent_test(self, num_requests: int, concurrency: int, 
                                test_providers: List[str]) -> List[Dict[str, Any]]:
        """运行并发测试"""
    ...
    def analyze_results(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for provider in set(r['provider'] for r in results):
            ...
            provider_stats[provider] = {
                ...
                'p95_duration': statistics.quantiles(provider_durations, n=20)[18] if len(provider_durations) > 1 else 0,
                'p99_duration': statistics.quantiles(provider_durations, n=100)[98] if len(provider_durations) > 1 else 0
            }
