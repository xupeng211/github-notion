...
class MetricsAnalyzer:
    ...
    def analyze_metrics_completeness(self, parsed_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "completeness_ratio": len(found_metrics & expected_metrics) / len(expected_metrics) if expected_metrics else 1.0,
    ...
    def collect_metrics_over_time(self, duration_minutes: int = 5, 
                                  interval_seconds: int = 30) -> List[Dict[str, Any]]:
        """在一段时间内收集指标"""
        ...
        while time.time() < end_time:
            ...
            if metrics_data["status"] == "success":
                print(f"✅ {metrics_data['timestamp'].strftime('%H:%M:%S')} - 指标收集成功 "
                      f"({len(metrics_data['parsed_metrics'])} 个指标)")
            else:
                print(f"❌ {metrics_data['timestamp'].strftime('%H:%M:%S')} - 指标收集失败: "
        ...
        for metric_name in self.key_metrics.keys():
            ...
            for snapshot in metrics_snapshots:
                ...
                if metric_name in parsed_metrics:
                    ...
                    metric_values_over_time.append({
            ...
            if len(metric_values_over_time) >= 2:
                ...
                trends[metric_name] = {
                    ...
                    "relative_change": ((final_value - initial_value) / initial_value * 100) if initial_value > 0 else 0,
                    "data_points": len(metric_values_over_time),
                    "trend": "increasing" if final_value > initial_value else ("decreasing" if final_value < initial_value else "stable")
                }
    ...
    def generate_analysis_report(self, include_trends: bool = True) -> str:
        ...
        report.append(f"  核心指标覆盖: {completeness['found_key_metrics']}/{completeness['expected_metrics']} "
