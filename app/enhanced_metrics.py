...
) if not DISABLE_METRICS else _NoopMetric()
...
# Notion API 调用
NOTION_API_CALLS_TOTAL = Counter(
...
DEADLETTER_REPLAYS_TOTAL = Counter(
...
def record_api_call(api_type: str, method: str, endpoint: str, status_code: int, duration: float,
    ...
    if api_type == "github":
        GITHUB_API_CALLS_TOTAL.labels(
            method=method,
            endpoint=endpoint,
            status_code=str(status_code)
        ).inc()

        GITHUB_API_DURATION.labels(
            method=method,
            endpoint=endpoint
        ).observe(duration)
        ...
        NOTION_API_CALLS_TOTAL.labels(
            method=method,
            endpoint=endpoint,
            status_code=str(status_code)
        ).inc()

        NOTION_API_DURATION.labels(
            method=method,
            endpoint=endpoint
        ).observe(duration)
