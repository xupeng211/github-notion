...
class IdempotencyMonitoringStressTester:
    ...
    async def send_webhook_request(
        self, session: aiohttp.ClientSession, test_id: int, is_duplicate: bool = False
    ) -> TestResult:
        """发送单个 webhook 请求"""
            ...
            async with session.post(
                ...
                return TestResult(
                    ...
                    duplicate_detected=(
                        ...
                        or "replay_attack_detected" in response_text.lower()
                        or response.status == 403  # 重复请求通常返回403
                    ),
    ...
    async def monitor_metrics(self, session: aiohttp.ClientSession, duration_seconds: int):
        ...
        while time.time() < end_time:
            ...
            if "error" not in metrics:
                metrics_history.append({"timestamp": datetime.now(), "metrics": metrics})

            await asyncio.sleep(self.test_config["metrics_check_interval"])

        return metrics_history

    async def run_idempotency_stress_test(self, concurrent_requests: int, total_requests: int) -> Dict[str, Any]:
        """运行幂等性压力测试"""
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            # 启动指标监控
            monitor_task = asyncio.create_task(self.monitor_metrics(session, duration_seconds=60))

            # 创建请求任务
            tasks = []
            for i in range(total_requests):
                is_duplicate = i % int(1 / self.test_config["duplicate_rate"]) == 0
                task = asyncio.create_task(self.send_webhook_request(session, i, is_duplicate))
                tasks.append(task)
    ...
    def evaluate_test_results(self, results: Dict[str, Any]) -> bool:
        ...
        if summary["idempotency_accuracy"] >= criteria["idempotency_accuracy"]:
            passed_checks.append(
                f"✅ 幂等性准确率: {summary['idempotency_accuracy']:.2%} >= {criteria['idempotency_accuracy']:.1%}"
            )
        else:
